...
methods. There is nothing particularly novel about these drivers, so we will
move on.

The Network Server and Network Client drivers provide parity with AFL when it
is combined with Preeny.  We found we needed two drivers, as one will establish
a connection to a server, while the other will accept a connection from a
client. These drivers are what caused the creation of the multipart mutator,
which is covered in more detail in section \ref{Mutator}.

We created a Windows Media Player (WMP) driver to demonstrate how to deal with
...
%       so we can compare several libraries.  The graphs can be generated in
