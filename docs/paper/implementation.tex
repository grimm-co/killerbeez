...
modular drivers Killerbeez can actually deal with this problem instead of simply avoiding it.
...
radamsa mutator module in Killerbeez is just a simple wrapper which feeds data
...
execute main() once per input.  This is much faster
...
binaries. It is essential in feedback-based fuzzing because it helps expand code
coverage by keeping only the inputs which reach new code.
...
module can do only lightweight tracking, as it done in \AFL{}, or it can
...
information, and so forth. The target function must be identified outside of
...
overhead, so the Intel PT instrumentation modules does not do it. Instead it just
...
A tracer is just an instrumentation module which captures detailed trace
