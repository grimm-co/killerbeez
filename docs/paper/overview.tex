...
% TODO: Fix the client box which has the "target" listed twice, and put
% a dashed line on the left of the legend so it is in a box instead of
% in 3/4 of a box
\end{multicols}
\begin{figure}[hp]
\centering
...
% TODO: start with the explanation of what the point of the manager is, not
%       with details about what platform it runs on.  In fact, the platform
%       thing might be something which should be moved to the implementation
%       section
% Select which mutators to run, which instrumentation to use?, how much CPU
% power to allocate to various targets, weight seeds?, finding unique new
% paths (when two nodes independently find inputs which hit the same new path,
% only one of them should be kept
...
%       to the system, such as the tracer, Driller, Synfuzz, and so on

% TODO: Mention the tracer here
...
% TODO: This graphic looks ugly with the boxes around the arrow labels
%       Not all arrows are labeled (e.g. fuzzer <-> driver)
%       It should be fuzzer, not fuzzer.exe
\end{multicols}
\begin{figure}[hp]
\centering
