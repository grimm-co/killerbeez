...
queue up jobs to be served out to clients, process the results which come back to
...
The API can also be used for some of the manager's internal functionality. The
campaign manager, which plans out jobs to be
executed, consists of pluggable modules to choose the job's type, inputs, and
parameters. The API can be used by these modules to gather the data they need,
and by the campaign manager to submit the final job. The corpus
minimization can also be dealt with via this interface by making calls to read
execution traces and modify the working set. This allows these components to be
