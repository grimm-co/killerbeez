...
\api{
int mutate\_thread\_safe(void * mutator\_state, char * buffer, size\_t buffer\_length)
}{
This function is identical to the \texttt{mutate} function, with the exception
it ensures that the mutations are done in a thread safe way.  If the mutator
will be accessed via multiple concurrent threads, this function should be used
instead of the \texttt{mutate} function.
}{
\item mutator\_state - a mutator specific structure previously created by the
create() function.
\item buffer - a buffer to which the mutated input will be written
...
\api{
int mutate\_input\_part(void * mutator\_state, int input\_part, char * buffer, size\_t buffer\_length)
}{
This function is similar to the \texttt{mutate} function, however it allows the
user to specify which part of the input should be mutated.  For some of the
fuzzer applications, it may be necessary to split the input up into separate
pieces that are mutated independently.  In these cases, the mutator can be given
...
network packet sent to the target process.
}{
\item mutator\_state - a mutator specific structure previously created by the
create() function.
\item input\_part - an index into the list of inputs that a mutator is mutating.
\item buffer - a buffer to which the mutated input will be written
\item buffer\_length - the size of the passed in buffer argument
\item return value - the length of the mutated data, 0 when the mutator is out
of mutations, or -1 on error or if the specified input\_part index is larger the
number of input parts that the mutator has.
}
...
This function will retrieve information about the number of inputs and the size
of each input that is managed by a mutator.
For most of the simple mutators, they will only be given
a single input buffer.  However, some of the more complicated mutators, such as
the manager mutator, will manage several input buffers and mutate them
independently with the \texttt{mutate\_input\_part} function.  This function
