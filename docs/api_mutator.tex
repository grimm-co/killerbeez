...
that it accepts flags parameter that specifies how the mutations should
be done.
}{
\item mutator\_state - a mutator specific structure previously created by the
create() function.
...
concurrent threads, this flags should be set.
\item \texttt{MUTATE\_MULTIPLE\_INPUTS} - If the mutator will be handling
individual inputs parts, this flags should be used.  For some of the fuzzer
